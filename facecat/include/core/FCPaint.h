...
#ifdef __FCPAINT_H__
#else                                                                            
#define __FCPAINT_H__ _declspec(dllexport) 
#endif
#pragma once
#include "..\\..\\stdafx.h"
...
namespace FaceCat{
	...
	private:
		...
		*/
		int hashKey(wstring pKey){
			string sKey = FCTran::StringTostring(pKey);
	...
	* 视图内容的布局
	*/
	enum __FCPAINT_H__ FCContentAlignment{
	...
	* 视图绑定边缘类型
	*/
	enum __FCPAINT_H__ FCDockStyle{
	...
	* 视图横向排列方式
	*/
	enum __FCPAINT_H__ FCHorizontalAlign{
	...
	* 视图纵向排列方式
	*/
	enum __FCPAINT_H__ FCVerticalAlign{
	...
	* 视图布局样式
	*/
	enum __FCPAINT_H__ FCLayoutStyle{
	...
	* 镜像模式
	*/
	enum __FCPAINT_H__ FCMirrorMode{
	...
	* 边距
	*/
	struct __FCPAINT_H__ FCPadding{
	public:
	    /**
		 * 底边距
		 */
	...
	* 触摸信息
	*/
    class __FCPAINT_H__ FCTouchInfo{
    public:
		/*
		* 点击次数
		*/
